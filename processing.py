...
def frame_capture(file_path, sample_frequency):
    print(file_path)
...
def window(video, sample_frequency, frame):
    total_frames = video.get(cv2.CAP_PROP_FRAME_COUNT)
    left = frame - (sample_frequency // 3) if frame - (sample_frequency // 3) > 0 else 0
    right = frame + (sample_frequency // 3) if frame + (sample_frequency // 3) < total_frames - 1 else total_frames
    ...
    while curr_frame_number < right:
        ...
        if success:
            ...
            if smallest is None or np.sum(temp_diff) < smallest:
...
directory = "./invention_test_videos/"

for file in listdir(directory):
    frame_capture(directory + file, 1)
